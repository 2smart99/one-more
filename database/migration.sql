...
-- 3. Fix RLS policies
-- The old policies used current_setting('app.user_id') which is never set by the
-- Supabase JS client over HTTP. Replace with permissive read + write via service role.
...
-- Also grant to authenticated role (service role bypasses RLS anyway)
